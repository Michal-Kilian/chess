...
export const calculateEvaluation = (
  ...
  const whiteMaterialLoss: number = capturedWhitePieces.reduce((acc: number, piece: Piece): number => acc + piece.value, 0);
  const blackMaterialLoss: number = capturedBlackPieces.reduce((acc: number, piece: Piece): number => acc + piece.value, 0);
  ...
  return {
    winning: materialDifference > 0 ? "white" : materialDifference < 0 ? "black" : "equal",
