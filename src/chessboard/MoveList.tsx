    ...
    <div ref={setMoveListRef}
      ...
      <Switch>
        <Match when={props.moves().length === 0}>
              <span
                class="min-h-8 h-8 px-2 py-1 w-full flex items-center justify-start border-b border-slate-700 text-slate-200">
                Initial position
              </span>
        </Match>
      ...
      <For each={Array.from({ length: Math.ceil(props.moves().length / 2) }, (_: unknown, i: number): number => i)}>
        {(index: number) => (
          <div
            class="flex flex-row items-center justify-between text-sm w-full h-fit px-2 py-1 border-b border-slate-700">
